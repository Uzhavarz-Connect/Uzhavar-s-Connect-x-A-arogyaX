...
export default function MedChatbot() {
  ...
  const [isMediaSelectorOpen, setIsMediaSelectorOpen] = useState(true);
  const [input, setInput] = useState<string>("");
  const [isRecording, setIsRecording] = useState(false);

  useEffect(() => {
    const fetchSessionId = async () => {
      const id = await ServerHandler.getMedbotSessionId();
      setSessionId(id);
    };
    fetchSessionId();

    return () => {
      if (sessionId) ServerHandler.closeMedbot(sessionId);
    };
  }, []);
  ...
  const handleSendMessage = async () => {
    ...
    if (sessionId) {
      const botResponse = await ServerHandler.sendMedbotMessage(
        sessionId,
        input
      );
